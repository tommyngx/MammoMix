...
os.environ["TF_CPP_MIN_LOG_LEVEL"] = (
    "3"  # Tắt log của TensorFlow (0=verbose, 1=info, 2=warning, 3=error)
)
...
def test_moe_model(moe_model_path, expert_dir, test_dataset, image_processor):
    ...
    try:
        ...
        eval_loader = DataLoader(
            test_dataset, batch_size=2, collate_fn=collate_fn, shuffle=False
        )
        ...
        for i in range(len(all_predictions)):
            ...
            batch_pred_boxes = torch.zeros(
                batch_logits.shape[0], batch_logits.shape[1], 4
            ).numpy()
        ...
        print(
            f"DEBUG: First prediction batch shape: {predictions_formatted[0][1].shape}"
        )
        print(
            f"DEBUG: First prediction values sample: {predictions_formatted[0][1][0, :3, :]}"
        )
        ...
        if len(targets_formatted[0]) > 0:
            ...
            print(
                f"DEBUG: First target boxes: {targets_formatted[0][0].get('boxes', [])}"
            )
...
def main(
    ...
    DATASET_NAME = (
        ...
        else config.get("dataset", {}).get("name", "CSAW")
    )
    ...
    test_loader = DataLoader(test_dataset, batch_size=2, collate_fn=collate_fn)
    sample_batch = next(iter(test_loader))
    pixel_values = sample_batch["pixel_values"].to(
    ...
    if moe_model and os.path.exists(moe_model):
        if weight_dir:
            expert_dir = os.path.dirname(
                weight_dir
            )  # Parent directory containing all experts
            try:
                moe_results = test_moe_model(
                    moe_model, expert_dir, test_dataset, image_processor
                )
            except Exception as e:
            ...
            print(
                "Warning: weight_dir required for MoE testing (to find expert models)"
            )
    elif moe_model:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--epoch", type=int, default=None, help="Dataset epoch value to pass to dataset"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to model folder containing config.json, model.safetensors, preprocessor_config.json",
    )
    parser.add_argument(
        "--num_samples",
        default=8,
        help='Number of test samples to use (or "all" for full dataset)',
    )
    parser.add_argument(
        "--moe_model", type=str, default=None, help="Path to trained MoE model file"
    )
